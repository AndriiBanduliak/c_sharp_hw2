...
int number = int.Parse(ReadLine()); // если 22 строчку удаляете, - то просто вместо ридлайна надо написать номер числа, которое нужно вывести
...
if (digitCount(digit) <= -number || digitCount(digit) >= number)
{
    ...
    WriteLine($"{number} number is  {digit}");
}
else WriteLine($"{number} number is not included {digit}");